...
final class DepositCryptoRecordDetailViewTests: XCTestCase {
  ...
  func test_PaymentStatusIsFloating_ShouldDisplayUploadFileLink_KTO_TC_93() {
    ...
    let records = stubViewModel.generateRecords(self.generateCryptoLog(status: .floating))

    let hasLink = records
      .filter {
        if
          case DepositCryptoRecord.link(let item) = $0,
          item.attachment != nil
        {
          return true
        }
  ...
  func test_PaymentStatusIsNotApprove_FieldShouldDisplayDash_KTO_TC_94() {
    ...
    let records = stubViewModel.generateRecords(self.generateCryptoLog(status: .pending))

    let filtered = records
      .compactMap { record -> [DepositCryptoRecord.Item]? in
