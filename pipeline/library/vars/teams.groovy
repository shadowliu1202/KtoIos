def notifyQat3(def teamsTokenId, def versionCore, def preRelease, def newBuildNumber,def oldBuildNumber, def testFlightLink) {
    ...
    wrap([$class: 'BuildUser']) {
        withCredentials([string(credentialsId: "$teamsTokenId", variable: 'token')]) {
           office365ConnectorSend webhookUrl: "$token",
                                ...
                                factDefinitions: [[name: 'testflight page', template: "<a href=\"$testFlightLink\">link</a>"],
                                                  [name: 'hotfix issues', template: "[$versionCore] update from $oldBuildNumber to $newBuildNumber (<a href=\"$updateIssues\">issues</a>)"],
