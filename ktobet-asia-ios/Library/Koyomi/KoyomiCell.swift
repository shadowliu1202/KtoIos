...
final class KoyomiCell: UICollectionViewCell {
  ...
  enum CellStyle {
    case standard
    case circle
    case semicircleEdge(position: SequencePosition)
    case line(position: SequencePosition?)
  ...
  var textColor = UIColor.KoyomiColor.black {
    didSet {
    ...
    model: DateModel)
  {
    ...
    case .semicircleEdge(let position):
      ...
      if case .left = position {
        ...
        leftSemicircleView.backgroundColor = .complementaryDefault.withAlphaComponent(0.3)
        rightSemicircleView.backgroundColor = .complementaryDefault.withAlphaComponent(0.3)
...
extension KoyomiCell {
  private var postion: CGPoint {
  ...
  private func adjustSubViewsFrame() {
    ...
    xLabel.frame = CGRect(
      x: postion.x,
      y: postion.y + contentLabel.frame.height - 2,
