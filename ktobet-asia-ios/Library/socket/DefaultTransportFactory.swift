...
internal class DefaultTransportFactory: TransportFactory {
  ...
  init(logger: SocketLogger, permittedTransportTypes: TransportType = .all) {
    self.logger = logger
  ...
  /// Sets the chosen type to have lowest priority for future reconnect attempts, to allow fallback when a transport does not work
  /// properly, e.g. due to network conditions.
