...
public class WebsocketsTransport: Transport {
  ...
  public func start(url: URL, options: HttpConnectionOptions) {
    ...
    webSocket!.event.error = { [weak self] error in
      ...
      // This handler should not be called after the close event but we need to mark the transport as closed to prevent calling
      // transportDidClose
  ...
  @inline(__always)
  private func setAccessToken(accessTokenProvider: () -> String?, request: inout URLRequest) {
