...
class OfflinePaymentViewModel:
    ...
    @Published private(set) var remitterName: String = ""
    @Published private(set) var remitAmountLimitRange: String = ""
    ...
    @Published private(set) var submitButtonDisable: Bool = true
       ...
       stream
            ...
            .subscribe(onNext: { [weak self] bankCardDTOs in
                ...
                self.gateways = bankCardDTOs
                    .map { bankCardDTO in
                        return OfflinePaymentDataModel.Gateway(
    ...
    private func getRemitAmountLimitRange(_ stream: Observable<PaymentsDTO.Offline>) {
        stream
            .map { offlineDTO in
                String(format: Localize.string("deposit_offline_step1_tips"),
    ...
    private func createOfflineDepositMemo(_ info: OfflinePaymentDataModel.RemittanceInfo) {
        guard let selectedGatewayId = info.selectedGatewayId,
