...
extension Wallet.VerifyStatus {
  func statusConfig(isCrypto: Bool) -> (text: String, color: UIColor) {
    switch self {
    case .pending:
      return (Localize.string("withdrawal_bankcard_new"), .gray9B9B9B)
    case .void_:
      return (Localize.string("withdrawal_bankcard_fail"), .redF20000)
    case .onhold:
      return (Localize.string("withdrawal_bankcard_locked"), .orangeFF8000)
    case .verified:
      return (Localize.string(isCrypto ? "cps_account_status_verified" : "withdrawal_bankcard_verified"), .green6AB336)
    default:
      return ("", .clear)
    }
  }
