  ...
  private func gatewayCell(name: String, identity: String, isLastCell: Bool) -> some View {
    VStack(spacing: 0) {
      HStack(spacing: 0) {
        ...
        LimitSpacer(8)
        Image(
          selectedItem == nil ? "iconSingleSelectionEmpty24" : identity == selectedItem!
            .identity ? "iconSingleSelectionSelected24" : "iconSingleSelectionEmpty24")
          .resizable()
          .scaledToFit()
  ...
  struct Preview: View {
    let gateways = [
          ...
          cash: CashType
            .Option(
              ...
              limitation: AmountRange(
                min: FiatFactory.shared
                  .create(simpleName: "", amount: BignumBigDecimal.companion.fromInt(int: 200)),
                max: FiatFactory.shared.create(simpleName: "", amount: BignumBigDecimal.companion.fromInt(int: 200)))),
      ...
      PaymentsDTO.Gateway(
        identity: "20",
        name: "JinYiCrypto",
        cash: CashType.Option(
          list: [],
          limitation: AmountRange(
            min: FiatFactory.shared.create(simpleName: "", amount: BignumBigDecimal.companion.fromInt(int: 200)),
            max: FiatFactory.shared.create(simpleName: "", amount: BignumBigDecimal.companion.fromInt(int: 200)))),
        remitType: PaymentsDTO.RemitType.normal,
        remitBank: [],
        verifier: CompositeVerification<RemitApplication, PaymentError>(),
    ...
    @State private var selectedGateway: PaymentsDTO.Gateway? = PaymentsDTO.Gateway(
      identity: "70",
      name: "JinYiDigital",
      cash: CashType.Option(
        list: [],
