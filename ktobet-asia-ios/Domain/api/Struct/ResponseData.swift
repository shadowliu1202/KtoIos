...
struct CasinoData: Codable {
    ...
    func toCasinoGame(host: String) throws -> CasinoGame {
        let thumbnail = CasinoThumbnail(host: host, thumbnailId: imageID)
        return try CasinoGame(
            gameId: Int32(gameID),
            gameName: name,
            isFavorite: isFavorite,
            gameStatus: GameStatus.Companion().convert(gameMaintenance: isGameMaintenance, status: status),
            thumbnail: thumbnail,
...
struct RecentGameBean: Codable {
    ...
    func toSlotGame(portalHost: String) -> SlotGame {
        SlotGame(
            ...
            thumbnail: SlotThumbnail(host: portalHost, thumbnailId: imageId),
            hasForFun: hasForFun,
            jackpotPrize: 0
        )
    }
...
struct SlotBetSummaryBean: Codable {
    ...
    struct Summary: Codable {
        ...
        func toDateSummary() throws -> DateSummary {
            try DateSummary(
                totalStakes: stakes.toAccountCurrency(),
                totalWinLoss: winloss.toAccountCurrency(),
...
struct SlotNewAndJackpotBean: Codable {
    ...
    func toSlotNewAndJackpotGames(portalHost: String) -> SlotNewAndJackpotGames {
        SlotNewAndJackpotGames(
            newGame: newGame.map { $0.toSlotGame(portalHost: portalHost) },
            jackpotGames: jackpot.map { $0.toSlotGame(portalHost: portalHost) }
        )
    }
}
...
struct SlotDateGameRecordBean: Codable {
    ...
    init(gameId: Int32, gameList: [SlotDateGameRecordBean]) {
        count = gameList.map { $0.count }.reduce(0, +)
        endDate = gameList.max { a, b -> Bool in a.endDate < b.endDate }?.endDate ?? ""
        self.gameId = gameId
        gameName = gameList.first?.gameName ?? ""
        imageId = gameList.first?.imageId ?? ""
        stakes = gameList.map { $0.stakes }.reduce(0, +)
        startDate = gameList.min(by: { a, b -> Bool in a.startDate < b.startDate })?.startDate ?? ""
        winloss = gameList.map { $0.winloss }.reduce(0, +)
    }
...
struct NumberGameEntity: Codable {
    ...
    func toNumberGame(portalHost: String) -> NumberGame {
        NumberGame(
            ...
            gameName: gameName,
            isFavorite: isFavorite,
            gameStatus: GameStatus.Companion().convert(gameMaintenance: isMaintenance, status: gameStatus),
...
struct RecordSummary: Codable {
    ...
    func toNumberGame() throws -> NumberGameSummary.Date {
        try NumberGameSummary.Date(
            betDate: betDate.toLocalDateWithAccountTimeZone(),
            count: count,
            stakes: stakes.toAccountCurrency(),
            winLoss: winLoss.toAccountCurrency()
        )
...
struct LockedBonusDataBean: Codable {
    ...
    func toTurnOverDetail() throws -> TurnOverDetail {
        try TurnOverDetail(
            achieved: achieved.currencyAmountToDouble()?.toAccountCurrency() ?? 0.toAccountCurrency(),
            formula: formula, informPlayerDate: informPlayerDate.toOffsetDateTime(),
            name: name,
            bonusId: no,
            remainAmount: remainingAmount.currencyAmountToDouble()?.toAccountCurrency() ?? 0.toAccountCurrency(),
            parameters: toTurnOverDetailParameters(parameters)
        )
    }
...
struct ArcadeGameBetRecordBean: Codable {
    ...
    func toArcadeGameBetRecord() throws -> ArcadeGameBetRecord {
        try ArcadeGameBetRecord(
            wagerId: wagerId,
            betId: betId,
            betTime: betTime.toOffsetDateTime(),
            settleTime: settleTime.toOffsetDateTime(),
            hasDetails: hasDetails,
            stakes: stakes.toAccountCurrency(),
            winLoss: winLoss.toAccountCurrency()
        )
...
extension BonusBean.Coupon {
    var couponStatus: CouponStatus {
        covertBonusPromotionStatus(bonusCouponStatus, isLimitedByDailyFull)
    }

    var knAmount: AccountCurrency {
        amount.toAccountCurrency()
    }

    var knMaxAmount: sharedbu.Promotion.IMaxAmount {
        sharedbu.Promotion.companion.create(amount: maxAmount.toAccountCurrency())
    }

    var knMinCapital: AccountCurrency {
        minCapital.toAccountCurrency()
    }
    ...
    func toBonusCoupon() throws -> BonusCoupon {
        switch type {
        case 1:
            return try toFreebet()
        case 2:
            return try toDepositReturnCustomize()
        case 3:
            return try toProduct()
        case 4:
            return try toRebate()
        case 5:
            return try toDepositReturnLevel()
        case 7:
            return try toVVIPCashback()
        default:
    ...
    private func toFreebet() throws -> BonusCoupon {
        try BonusCoupon.FreeBet(
            promotionId: displayId,
            bonusId: no,
            name: name,
            couponStatus: couponStatus,
            percentage: knPercentage,
            amount: knAmount,
            maxAmount: knMaxAmount,
            betMultiple: betMultiple,
            fixTurnoverRequirement: fixTurnoverRequirement,
            informPlayerDate: informPlayerDate.toOffsetDateTime(),
            updatedDate: updatedDate.toKotlinLocalDateTime(),
            validPeriod: ValidPeriod.Companion().create(
                start: effectiveDate?.toOffsetDateTime() ?? OffsetDateTime.companion.NotDefine,
                end: expiryDate?.toOffsetDateTime() ?? OffsetDateTime.companion.NotDefine
            ),
            minCapital: knMinCapital
        )
    }

    private func toDepositReturnCustomize() throws -> BonusCoupon {
        try BonusCoupon.DepositReturnCustomize(property: toDepositReturnProperty())
    }
    ...
    private func toVVIPCashback() throws -> BonusCoupon.VVIPCashback {
        try BonusCoupon.VVIPCashback(property: BonusCoupon.Property(
            ...
            bonusId: no,
            name: name,
            issueNumber: KotlinInt(value: Int32(issueNumber)),
            percentage: knPercentage,
            amount: knAmount,
            endDate: effectiveDate?.toKotlinLocalDateTime() ?? OffsetDateTime.companion.NotDefine.toInstant().toLocalDateTime(timeZone: Injectable.resolve(PlayerConfiguration.self)!.localeTimeZone().toKotlinTimeZone()),
            betMultiple: betMultiple,
            fixTurnoverRequirement: fixTurnoverRequirement,
            validPeriod: ValidPeriod.Companion().create(
                start: effectiveDate?.toOffsetDateTime() ?? OffsetDateTime.companion.NotDefine,
    ...
    func toCashbackPromotion() throws -> PromotionEvent.VVIPCashback {
        try PromotionEvent.VVIPCashbackCompanion()
            .create(
                promotionId: displayId,
                issueNumber: issue,
                informPlayerDate: informPlayerDate.toKotlinLocalDateTime(),
                percentage: Percentage(percent: percentage),
                maxBonusAmount: maxAmount.toAccountCurrency(),
    ...
    func toProductPromotion() throws -> PromotionEvent.Product {
        try PromotionEvent.ProductCompanion()
            .create(
                ...
                endDate: endDate.toOffsetDateTime(),
                maxBonusAmount: maxAmount.toAccountCurrency(),
                type: ProductType.convert(productType)
            )
    }
    ...
    struct Parameters: Codable {
        ...
        fileprivate func toTurnOverHintParameters() -> TurnOverHint.Parameters {
            TurnOverHint.Parameters(
                amount: toCashAmount(amount),
                balance: toCashAmount(balance),
                betMultiplier: Int32(betMultiplier ?? 0),
                capital: toCashAmount(capital),
                request: toCashAmount(request),
                requirement: toCashAmount(requirement)
            )
        }
    }

    func toTurnOverHint() -> TurnOverHint {
        TurnOverHint(formula: formula, parameters: parameters.toTurnOverHintParameters())
    }
}
...
struct PromotionTemplateBean: Codable {
    ...
    func toPromotionDescriptions() -> PromotionDescriptions {
        PromotionDescriptions(content: contentTemplate, rules: rulesTemplate)
    }
}
