...
struct WithdrawalRecordDetailData: Codable {
  ...
  func toWithdrawalDetail(
    transactionTransactionType: TransactionType,
    statusChangeHistories: [Transaction.StatusChangeHistory],
  ...
  private func toCryptoExchangeRecord(
    _ cryptoCurrency: String,
    _ cryptoCurrencyCode: Int,
    _ accountCurrency: String,
    _ exchangeRate: String,
    _ createdDate: String,
    playerLocale: SupportLocale) throws
    -> CryptoExchangeRecord
  {
    do {
...
struct WithdrawalAccountBean: Codable {
  ...
  class BankCardModle: BankCard {
    ...
    var verifyStatus: PlayerBankCardVerifyStatus {
      PlayerBankCardVerifyStatus.Companion().create(status: Int32(bean.verifyStatus))
    }
    ...
    func createBankName(bankName: String, bank: Bank?, locale _: SupportLocale) -> String {
  ...
  func toFiatBankCard(banks: [Int: Bank], locale: SupportLocale) -> FiatBankCard {
    FiatBankCard(
      bankCard: BankCardModle(self, banks, locale),
...
struct CryptoBankCardBeanCodable: Codable {
  ...
  func toCryptoBankCard() throws -> CryptoBankCard {
    ...
    let bankCard = BankCardObject(
      id_: playerCryptoBankCardId,
      name: cryptoWalletName,
      status: status,
      verifyStatus: PlayerBankCardVerifyStatus.Companion().create(status: verifyStatus))
    ...
    return CryptoBankCard(
      ...
      cryptoNetwork: convertTo(cryptoNetwork: cryptoNetwork))
  }
...
struct CryptoWithdrawalTransaction: Codable {
  ...
  private func sumOfTurnOver(infos: [CryptoWithdrawalRequestInfo]) -> AccountCurrency {
    infos.map({ $0.withdrawalRequest.toAccountCurrency() }).reduce(AccountCurrency.zero()) { $0 + $1 }
  }
...
struct Log: Codable {
  ...
  func toBalanceLog(transactionFactory: TransactionFactory) throws -> TransactionLog {
    let transactionTypes = TransactionTypes.Companion().create(type: transactionType)
    let productTypes = ProductType.convert(productType)
    let bonusTypes = BonusType.convert(bonusType)
    let productProviders = ProductProviders.Companion().createProductGroup(provider: productProvider)
