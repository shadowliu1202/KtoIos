...
class FilterButton: UIView {
  typealias ConditionCallbck = ([FilterItem]) -> Void
  ...
  @IBAction
  private func btnFilterPressed(_: UIButton) {
    goToFilterVC()
  }
  ...
  func setPromotionStyleTitle(source: [FilterItem]?) -> Self {
    var text = ""
    if isSelectedAllFilterItem(source: source) {
      text = (source?[PromotionPresenter.sortingRow].title ?? Localize.string("bonus_orderby_desc")) + "、" + Localize
        .string("common_all")
    }
    else {
      text = filterItemToTitleString(source: source)
