...
class PromotionFilterDropDwon: UIView {
  ...
  private lazy var arrowIcon = UIImageView(image: UIImage(named: "promotionArrowDropDown"))
  ...
  lazy var productTags: [PromotionProductTag] = [
    PromotionProductTag(isSelected: true, filter: .Sport),
  ...
  var clickHandler: ((PromotionTag?, [PromotionProductTag]) -> Void)?
  ...
  @objc
  private func touchAction(_: UIButton) {
    timer?.invalidate()
    let nextTimer = Timer.scheduledTimer(
      timeInterval: 0.3,
      target: self,
      selector: #selector(debounceAction),
      userInfo: nil,
      repeats: false)
    timer = nextTimer
  }
  ...
  @objc
  private func debounceAction(_: UIButton) {
    isExpand.toggle()
  }
  ...
  private func showList() {
    ...
    mainView.addSubview(mainStackView, constraints: [
  ...
  private func setupProductListConstrains(with tags: [PromotionProductTag]) {
    mainView.addSubview(line, constraints: [
      .constraint(.equal, \.trailingAnchor, offset: -30),
      .constraint(.equal, \.leadingAnchor, offset: 30),
    ...
    mainView.addSubview(bottomStackView!, constraints: [
      .constraint(.equal, \.trailingAnchor, offset: -30),
      .constraint(.equal, \.leadingAnchor, offset: 30)
    ])
  ...
  private func addBtnProductTags(stackView: UIStackView, data: [PromotionProductTag]) {
    ...
    for i in 0..<data.count {
      ...
      let icon = UIImageView(image: UIImage(named: "promotionTick"))
      contain.addSubview(icon, constraints: [
  ...
  @objc
  func pressFirstLayerTag(_ sender: UIButton) {
  ...
  @objc
  func pressSecondLayerTag(_ sender: UIButton) {
...
private struct PromotionFilterDropDwonRepresent: UIViewRepresentable {
