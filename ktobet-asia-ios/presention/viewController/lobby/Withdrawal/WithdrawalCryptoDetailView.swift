...
struct WithdrawalCryptoDetailView: View {
  ...
  private var finalInfoData: [(title: String, content: String)] {
    [
      ...
      (
        String(format: Localize.string("common_cps_final_amount"), data.actualCryptoAmount.cashAmount.simpleName),
        data.actualCryptoAmount.cashAmount.description()),
  ...
  private var applyInfoData: [(title: String, content: String)] {
    [
      ...
      (
        String(format: Localize.string("common_cps_apply_amount"), data.requestCryptoAmount.cashAmount.simpleName),
      ...
      VStack(alignment: .leading, spacing: 0) {
        ...
        VStack(alignment: .leading, spacing: 0) {
          WithdrawalCryptoDetailRowView(
            ...
            content: data.record.displayId,
            isShowBottomDivider: true)
          WithdrawalCryptoDetailRowView(
            title: Localize.string("common_status"),
            content: StringMapper.parse(
              data.record.transactionStatus,
              isPendingHold: data.isPendingHold,
              ignorePendingHold: true),
            isShowBottomDivider: true)
        }
        ...
        VStack(spacing: 0) {
          WithdrawalCryptoDetailRowView(
            title: String(format: Localize.string("common_cps_remitter"), Localize.string("cps_kto")),
            content: data.providerCryptoAddress,
            isShowBottomDivider: true)
          WithdrawalCryptoDetailRowView(
            title: String(format: Localize.string("common_cps_payee"), Localize.string("common_player")),
            content: data.playerCryptoAddress,
            isShowBottomDivider: true)
        ...
        if TransactionStatus.approved == data.record.transactionStatus {
          ForEach(finalInfoData.indices, id: \.self) { index in
            WithdrawalCryptoDetailRowView(
              ...
              isShowBottomDivider: false,
              contentColor: index == 0 ? Color(UIColor.orangeFF8000) : Color(.whitePure))
          }
        }
        else {
          ForEach(finalInfoData.indices, id: \.self) { index in
...
class WithdrawalCryptoPreviewData {
  var withdrawalRecord: WithdrawalRecord {
    WithdrawalRecord(
      transactionTransactionType: TransactionType.cryptowithdrawal,
      displayId: "displayId",
      transactionStatus: TransactionStatus.approved,
      createDate: Date().toUTCOffsetDateTime(),
      cashAmount: AccountCurrency(tempAmount: BignumBigDecimal.companion.fromInt(int: 200), simpleName: "", symbol: ""),
      isPendingHold: true,
      groupDay: SharedBu.LocalDate(year: 1999, month: SharedBu.Month.october, dayOfMonth: 1))
  }
  ...
  var requestCryptoAmount: CryptoExchangeRecord {
    ...
    let exchangeRate = CryptoExchangeFactory().create(from: cryptoType, to: locale, exRate: "0")
    return CryptoExchangeRecord(
      cryptoAmount: 0.toCryptoCurrency(SupportCryptoType.valueOf("ETH")),
      exchangeRate: exchangeRate,
      cashAmount: 3600.toAccountCurrency(),
      date: Date().toUTCOffsetDateTime())
  }
  ...
  var actualCryptoAmount: CryptoExchangeRecord {
    ...
    let exchangeRate = CryptoExchangeFactory().create(from: cryptoType, to: locale, exRate: "0")
    return CryptoExchangeRecord(
      cryptoAmount: 10.toCryptoCurrency(SupportCryptoType.valueOf("ETH")),
      exchangeRate: exchangeRate,
      cashAmount: 2030.toAccountCurrency(),
      date: Date().toUTCOffsetDateTime())
  }

  var statusChangeHistory: SharedBu.Transaction.StatusChangeHistory {
    SharedBu.Transaction.StatusChangeHistory(
      createdDate: Date().toUTCOffsetDateTime(),
      imageIds: [PortalImage.Public(imageId: "", fileName: "", host: "")],
