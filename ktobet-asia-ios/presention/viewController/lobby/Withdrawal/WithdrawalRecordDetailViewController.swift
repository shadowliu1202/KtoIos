...
class WithdrawalRecordDetailViewController: LobbyViewController {
  ...
  private func updateUI(detail: WithdrawalDetail.General) {
    DispatchQueue.main.async {
      ...
      self.statusLabel.text = StringMapper.parse(
        detail.record.transactionStatus,
        isPendingHold: detail.isPendingHold,
        ignorePendingHold: false)
  ...
  private func cofirmUploadImage() {
    self.viewModel.bindingImageWithWithdrawalRecord(
      displayId: displayId,
      transactionId: TransactionStatus.Companion().convertTransactionStatus(ticketStatus: .pending),
      portalImages: self.viewModel.uploadImageDetail.map { $0.value.portalImage })
  ...
  private func showImagePicker() {
    ...
    if currentSelectedImageCount >= WithdrawalViewModel.selectedImageCountLimit {
      Alert.shared.show(
        "",
        String(
          format: Localize.string("common_photo_upload_limit_reached"),
          "\(WithdrawalViewModel.selectedImageCountLimit)"),
        confirm: nil,
        cancel: nil)
    }
  ...
  private func addImageToUI(image: UIImage) {
    ...
    let removeButton = UIButton(frame: CGRect(
      x: self.imageStackView.frame.origin.x + CGFloat(12),
