...
class CryptoVerifyViewModel {
  lazy var phone = self.playerUseCase.loadPlayer()
    .do(onSuccess: { self.relayMobile.accept($0.playerInfo.contact.mobile ?? "") }).map { $0.playerInfo.contact.mobile }
  lazy var email = self.playerUseCase.loadPlayer().do(onSuccess: { self.relayEmail.accept($0.playerInfo.contact.email ?? "") })
    .map { $0.playerInfo.contact.email }
  ...
  func verify(playerCryptoBankCardId: String) -> Completable {
    withdrawalUseCase.sendCryptoOtpVerify(accountType: relayAccountType.value, playerCryptoBankCardId: playerCryptoBankCardId)
  }

  func verifyOtp(otp: String, accountType: AccountType) -> Completable {
    withdrawalUseCase.verifyOtp(verifyCode: otp, accountType: accountType)
  }

  func resendOtp() -> Completable {
    withdrawalUseCase.resendOtp(accountType: relayAccountType.value)
  }
