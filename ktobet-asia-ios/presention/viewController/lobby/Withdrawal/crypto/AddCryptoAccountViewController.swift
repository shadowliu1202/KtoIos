...
class AddCryptoAccountViewController: LobbyViewController {
  ...
  override func viewDidLoad() {
    ...
    accountAddressTextField.rx.observe(UIColor.self, "backgroundColor").bind(to: accountAddressView.rx.backgroundColor)
    ...
    submitButton.rx.tap.subscribe(onNext: { [weak self] in
      ...
      guard let self else { return }
      if self.bankCardCount >= Settings().WITHDRAWAL_CRYPTO_BANK_CARD_LIMIT {
        Alert.shared.show(
  ...
  override func handleErrors(_ error: Error) {
    if error is KtoWithdrawalAccountExist {
      Alert.shared.show(
        Localize.string("common_tip_title_warm"),
        Localize.string("cps_bank_card_exist"),
        confirm: nil,
        cancel: nil,
        tintColor: UIColor.redF20000)
    }
