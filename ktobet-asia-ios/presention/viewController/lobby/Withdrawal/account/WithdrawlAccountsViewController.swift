...
class WithdrawlAccountsViewController: LobbyViewController {
  ...
  fileprivate var switchAddAccount: (() -> Void)?
  ...
  private func cryptoDataBinding() {
    let cryptoDataSource = self.rx.viewWillAppear.flatMap({ [unowned self] _ in
      cryptoSource.asObservable()
    }).share(replay: 1)
    cryptoDataSource
      .catch({ [weak self] error -> Observable<[CryptoBankCard]> in
        self?.handleErrors(error)
        return Observable.just([])
      }).bind(to: tableView.rx.items) { [unowned self] tableView, _, item in
        tableView.dequeueReusableCell(withIdentifier: "CryptoAccountCell", cellType: CryptoAccountCell.self)
          .configure(item, self.isEditMode)
      }.disposed(by: disposeBag)
  ...
  private func generalDataBinding() {
    ...
    dataSource
      .catch({ [weak self] error -> Observable<[FiatBankCard]> in
        ...
        tableView.dequeueReusableCell(withIdentifier: "AccountCell", cellType: AccountCell.self)
          .configure(item, self.isEditMode)
      }.disposed(by: disposeBag)
  ...
  private func switchToAddAccount() {
    if self.withdrawalAccounts?.count ?? 0 < Settings().WITHDRAWAL_CASH_BANK_CARD_LIMIT {
      ...
      let msg = Localize.string("withdrawal_bankcard_add_overlimit", "\(Settings().WITHDRAWAL_CASH_BANK_CARD_LIMIT)")
      Alert.shared.show(title, msg, confirm: nil, cancel: nil)
    }
  }

  private func switchToAddCryptoAccount() {
    if cryptoSource.value.count >= Settings().WITHDRAWAL_CRYPTO_BANK_CARD_LIMIT {
      Alert.shared.show(
        Localize.string("common_tip_title_warm"),
        String(
          format: Localize.string("withdrawal_bankcard_add_overlimit"),
          "\(Settings().WITHDRAWAL_CRYPTO_BANK_CARD_LIMIT)"),
        confirm: nil,
        cancel: nil,
  ...
  @IBAction
  func unwindSegueAccount(segue _: UIStoryboardSegue) {
