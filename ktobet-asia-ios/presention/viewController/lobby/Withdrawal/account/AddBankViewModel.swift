...
class AddBankViewModel {
  ...
  lazy var bankValid: Observable<ValidError> = bankName.skip(InitAndKeyboardFirstEvent)
    .map { [unowned self] bankName -> ValidError in
      let result: BankNamePatternValidateResult = self.accountPatternGenerator.bankName(banks: self.bankNames)
        .validate(name: bankName)
  ...
  lazy var branchValid: Observable<ValidError> = branchName.skip(InitAndKeyboardFirstEvent)
    .map { [unowned self] text -> ValidError in
  ...
  lazy var areaName = AreaNameFactory.Companion().create(supportLocale: playerDataUseCase.getSupportLocalFromCache())
  ...
  lazy var provinceValid = province.skip(InitAndKeyboardFirstEvent).map({ [weak self] txt -> ValidError in
    if let self, self.isProvinceLegal(txt) {
      return .none
    }
    return .empty
  })
  ...
  lazy var countyValid: Observable<ValidError> = county.skip(InitAndKeyboardFirstEvent).map({ [weak self] txt in
    if let self, self.isCountyLegal(txt) {
      return .none
    }
    return .empty
  })
  private lazy var isCountyValid = countyValid.map({ $0 == .none ? true : false })
  func isCountyLegal(_: String) -> Bool {
  ...
  lazy var accontValid: Observable<ValidError> = account.skip(InitAndKeyboardFirstEvent)
    .map { [unowned self] text -> ValidError in
      if text.count == 0 {
        return .empty
      }
  ...
  lazy var btnValid: Observable<Bool> = Observable
    .combineLatest(isBankValid, isBranchValid, isProvinceValid, isCountyValid, isAccountValid) {
      $0 && $1 && $2 && $3 && $4
    }.startWith(false)
  ...
  func isRealNameEditable() -> Single<Bool> {
    playerDataUseCase.isRealNameEditable()
  }

  func getBanks() -> Single<[(Int, Bank)]> {
    bankUseCase.getBankMap().do(onSuccess: { [unowned self] tuple in
      self.bankNames = tuple.map { self.displayBank($0.1) }
    })
  }

  func getProvinces() -> [String] {
    areaName.getProvinces().map({ $0.name })
  }

  func getCountries(province: String) -> [String] {
    areaName.getCities(province: Province(name: province)).map({ $0.name })
  }
  ...
  func addWithdrawalAccount() -> Completable {
    ...
    let pureBankName = StringMapper.splitShortNameAndBankName(
      bankName: self.bankName.value,
      supportLocale: localStorageRepo.getSupportLocale())
    let newWithdrawalAccount = NewWithdrawalAccount(
      bankId: bankId,
      bankName: pureBankName,
      branch: self.branchName.value,
      location: self.province.value,
      city: self.county.value,
      address: "",
      accountNumber: self.account.value,
      accountName: self.userName.value)
