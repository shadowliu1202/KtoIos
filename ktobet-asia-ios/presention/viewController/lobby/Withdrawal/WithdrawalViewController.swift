...
class WithdrawalViewController: LobbyViewController {
  ...
  private lazy var dailyLimitAmount = "" {
    didSet {
      self.withdrawalTodayAmountLimitLabel.text = Localize.string(
        "withdrawal_daily_limit_widthrawal_amount",
        dailyLimitAmount)
    }
  }

  private lazy var dailyMaxCount = "" {
    didSet {
  ...
  private lazy var turnoverRequirement: AccountCurrency? = AccountCurrency.zero() {
    didSet {
      guard let turnoverRequirement else {
        self.turnoverRequirementLabel.text = Localize.string("withdrawal_turnover_requirement")
        return
      }
      let suffix = !turnoverRequirement.isPositive ? Localize.string("common_none") : Localize
        .string("common_requirement", "\(turnoverRequirement.formatString())")
  ...
  private lazy var crpytoWithdrawalRequirement: AccountCurrency? = AccountCurrency.zero() {
    didSet {
      ...
      if let crpytoWithdrawalRequirement, crpytoWithdrawalRequirement.isPositive {
        attribTextHolder = generateRequirementTextAttribute(
          requireAmount: crpytoWithdrawalRequirement.formatString(),
          cpsWithdrawalRequirement)
      }
      else {
  ...
  @IBAction
  func backToWithdrawal(segue: UIStoryboardSegue) {
  ...
  private func generateRequirementTextAttribute(requireAmount: String, _ prefix: String) -> AttribTextHolder {
    ...
    attribTextHolder = AttribTextHolder(text: fullText)
      ...
      .addAttr((text: fullText, type: .font, UIFont(name: "PingFangSC-Medium", size: 14)!))
    ...
    tips.bounds = CGRect(x: 0, y: 0, width: 7.4, height: 12)
  ...
  private func generateNotRequirementTextAttribute(_ prefix: String) -> AttribTextHolder {
    ...
    tips.bounds = CGRect(x: 0, y: -5, width: 20, height: 20)
    attribTextHolder = attribTextHolder.addAttr((text: "", type: .attachment, value: tips))
    return attribTextHolder
  }

  @objc
  private func tapOnRequirementTextView(_ sender: UITapGestureRecognizer) {
    ...
    if characterIndex < textView.textStorage.length {
      let link = textView.attributedText.attribute(
  ...
  @objc
  fileprivate func withdrawTap(_: UITapGestureRecognizer) {
    if let withdrawalLimits, withdrawalLimits.hasCryptoRequirement() {
      Alert.shared.show(
  ...
  @objc
  func crpytoTap(_: UITapGestureRecognizer) {
  ...
  fileprivate func withdrawalLimitationDataBinding() {
    self.rx.viewWillAppear.flatMap({ [unowned self] _ in
      self.viewModel.withdrawalAccounts().asObservable()
    ...
    self.rx.viewWillAppear.flatMap({ [unowned self] _ in
      self.viewModel.getWithdrawalLimitation().asObservable()
  ...
  @objc
  private func switchToCryptoTransationLog() {
  ...
  private func crpytoWithdrawalRequirementCurrencyName() -> String {
    self.withdrawalLimits?.unresolvedCryptoTurnover.denomination() ?? ""
  }

  private func crpytoWithdrawalRequirementAmount() -> AccountCurrency? {
    self.withdrawalLimits?.unresolvedCryptoTurnover
  }
  ...
  fileprivate func recordDataBinding() {
    ...
    }).bind(to: withdrawalRecordTableView.rx.items(
      cellIdentifier: String(describing: WithdrawRecordTableViewCell.self),
      cellType: WithdrawRecordTableViewCell.self))
    { _, data, cell in
  ...
  fileprivate func recordDataEvenhandler() {
    Observable.zip(
      withdrawalRecordTableView.rx.itemSelected,
      withdrawalRecordTableView.rx.modelSelected(WithdrawalRecord.self)).bind { [weak self] indexPath, data in
      guard let self else { return }
