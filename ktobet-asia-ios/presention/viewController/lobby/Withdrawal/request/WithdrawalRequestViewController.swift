...
class WithdrawalRequestViewController: LobbyViewController, AuthProfileVerification {
  ...
  private func initUI() {
    ...
    withdrawalAmountTextField.editingChangedHandler = { [unowned self] str in
      ...
      amount > maxmumAmount ? self.viewModel.relayWithdrawalAmount.accept(maxmumAmount.currencyFormatWithoutSymbol()) : self
        .viewModel.relayWithdrawalAmount
        .accept(strWithSeparator.contains(".") ? str : amount.currencyFormatWithoutSymbol(precision: 0))
    }
  ...
  private func withdrawalLimitationDataBinding() {
    viewModel.getWithdrawalLimitation().subscribe { [weak self] withdrawalLimits in
      ...
      self.withdrawalLimitLabel.text = String(
