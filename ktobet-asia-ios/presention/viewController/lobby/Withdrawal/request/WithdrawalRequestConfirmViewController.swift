...
class WithdrawalRequestConfirmViewController: LobbyViewController {
  ...
  private func initUI() {
    ...
    withdrawalTodayAmountLimitLabel.text = (withdrawalLimits.dailyCurrentCash - amount.toAccountCurrency())
      .formatString(.none)
    ...
    confirmButton.rx.tap.subscribe(onNext: { [weak self] in
      ...
      if self.viewModel.relayNameEditable {
        Alert.shared.show(
          String(format: Localize.string("withdrawal_success_confirm_title"), self.account.accountName),
