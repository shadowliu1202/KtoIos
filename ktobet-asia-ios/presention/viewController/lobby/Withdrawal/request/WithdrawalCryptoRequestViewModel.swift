...
class WithdrawalCryptoRequestViewModel {
  ...
  private func getCryptoWithdrawalLimits(
    _ cryptoType: SupportCryptoType,
    _ cryptoNetwork: CryptoNetwork) -> Single<WithdrawalLimits>
  {
    withdrawalUseCase.getCryptoWithdrawalLimits(cryptoType, cryptoNetwork)
  }

  func getWithdrawalLimitation() -> Single<WithdrawalLimits> {
    self.getCryptoWithdrawalLimits(self.cryptoType, self.cryptoNetwork)
  }
  ...
  func cryptoCurrency(cryptoCurrency: SupportCryptoType) -> Single<IExchangeRate> {
    self.withdrawalUseCase.getCryptoExchangeRate(cryptoCurrency)
  }
  ...
  func withdrawalAmountValidation() -> Observable<ValidError> {
    Observable.combineLatest(exchangeAmounts, getWithdrawalLimitation().asObservable(), getBalance().asObservable())
      .map { [weak self] amounts, limitation, balance -> ValidError in
  ...
  lazy var withdrawalValidation: Observable<Bool> = Observable
    .combineLatest(self.withdrawalAmountValidation(), self.getWithdrawalLimitation().asObservable())
    .map({ validError, limitation in
      validError == .none && limitation.dailyCurrentCount > 0
    }).startWith(false)
  ...
  func requestCryptoWithdrawal(
    ...
    -> Completable
  {
    withdrawalUseCase.requestCryptoWithdrawal(
  ...
  func fiatDecimalToAccountCurrency(_ de: Decimal) -> AccountCurrency {
    FiatFactory().create(supportLocale: self.localStorageRepository.getSupportLocale(), amount_: "\(de)")
  }

  func cryptoDecimalToCryptoCurrency(_ type: SupportCryptoType, _ de: Decimal) -> CryptoCurrency {
    CryptoFactory().create(supportCryptoType: type, amount_: "\(de)")
  }
