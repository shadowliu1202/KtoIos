...
class WithdrawalRequestViewModel {
  ...
  func getWithdrawalLimitation() -> Single<WithdrawalLimits> {
    self.withdrawalUseCase.getWithdrawalLimitation().do(onSuccess: { [weak self] limits in
  ...
  func isRealNameEditable() -> Single<Bool> {
    playerDataUseCase.isRealNameEditable().do(onSuccess: { [weak self] editable in
  ...
  func event() -> (
    dailyCountValid: Observable<Bool>,
    ...
    dataValid: Observable<Bool>)
  {
    ...
    let dataValid = Observable
      .combineLatest(dailyCountValid, dailyCashValid, amountValid, userNameValid) { $0 && $1 && $2 == .valid && $3 }
  ...
  func sendWithdrawalRequest(playerBankCardId: String, cashAmount: AccountCurrency) -> Single<String> {
    withdrawalUseCase.sendWithdrawalRequest(playerBankCardId: playerBankCardId, cashAmount: cashAmount)
  }
