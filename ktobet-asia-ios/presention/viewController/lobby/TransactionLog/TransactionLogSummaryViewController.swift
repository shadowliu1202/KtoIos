...
class TransactionLogSummaryViewController: LobbyViewController {
  ...
  override func viewDidLoad() {
    ...
    viewModel.getCashLogSummary(from: viewModel.dateType.result.from, to: viewModel.dateType.result.to)
      .do(onSuccess: { [weak self] summary in
        guard let self else { return }
        self.beginDateLabel.text = String(
          format: Localize.string("balancelog_summary_start_date"),
          self.viewModel.dateType.result.from.toDateString())
        self.beginAmountLabel.text = String(
          format: Localize.string("balancelog_summary_start_amount"),
          summary.previousBalance)
        self.endDateLabel.text = Date().convertdateToUTC() <= self.viewModel.dateType.result.to.convertdateToUTC() ?
          String(format: Localize.string("balancelog_summary_end_date"), Date().toDateString()) :
          String(format: Localize.string("balancelog_summary_end_date"), self.viewModel.dateType.result.to.toDateString())
        self.endAmountLabel.text = Date().convertdateToUTC() <= self.viewModel.dateType.result.to
          .convertdateToUTC() ? Localize
          .string("balancelog_summary_end_date_is_today") :
          String(format: Localize.string("balancelog_summary_end_amount"), summary.afterBalance)
      })
      .map({ summary -> [LogSummaryModel] in
        ...
        logSummary
          .append(LogSummaryModel(typeName: Localize.string("common_adjustment"), amount: summary.adjustmentAmount))
      ...
      .catch({ [weak self] in
        ...
        let cell = tableView
          .dequeueReusableCell(
            withIdentifier: TransactionLogSummaryTableViewCell
