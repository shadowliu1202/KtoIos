...
class DepositRecordDetailViewController: LobbyViewController {
  ...
  func confirm(_: Any) {
    ...
    viewModel.bindingImageWithDepositRecord(
      displayId: displayId,
      portalImages: viewModel.uploadImageDetail.map { $0.value.portalImage }).subscribe { [weak self] in
  ...
  @objc
  func openPhoto(_: UITapGestureRecognizer? = nil) {
    showImagePicker()
  }
    ...
    if currentSelectedImageCount >= DepositLogViewModel.selectedImageCountLimit {
      Alert.shared.show(
        ...
        String(
          ...
          "\(DepositLogViewModel.selectedImageCountLimit)"),
        confirm: nil,
        cancel: nil)
    }
    ...
    imagePickerView.showImageCountLimitAlert = { view in
      ...
      toastView.show(
        ...
        statusTip: String(
          ...
          String(DepositLogViewModel.selectedImageCountLimit - currentSelectedImageCount)),
  ...
  private func dataBinding() {
    ...
    shareStatusChangeHistories
      .catchError({ _ -> Observable<[UpdateHistory]> in
        ...
        cellType: RemarkTableViewCell.self))
    { [unowned self] _, d, cell in
