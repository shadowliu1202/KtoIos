...
class OfflineViewModel: CollectErrorViewModel, ViewModelType {
  ...
  init(
    ...
    accountPatternGenerator: AccountPatternGenerator,
    ...
    localStorageRepo: LocalStorageRepository)
  {
    ...
    let offlineBanks = offlinePayment.flatMapLatest { RxSwift.Single.from($0.beneficiaries) }
      .map { $0 as! [PaymentsDTO.BankCard] }
    ...
    let offlineDataValid = isOfflineDataValid(
    ...
    self.input = Input(
      selectPaymentGateway: self.selectPaymentGateway.asObserver(),
  ...
  private func getOfflinePaymentGateway(offlineBanks: Observable<[PaymentsDTO.BankCard]>)
    -> Driver<[OfflinePaymentGatewayItemViewModel]>
  {
    let offlinePaymentGateway = Observable.combineLatest(offlineBanks, bankUseCase.getBankMap().asObservable())
      .map { offline, banks -> [PaymentsDTO.BankCard] in
    ...
    return Observable.combineLatest(offlinePaymentGateway, selectPaymentGateway).map { banks, selectedBank in
      banks
        .map {
          OfflinePaymentGatewayItemViewModel(with: $0, icon: self.getIconNamed($0.bankId), isSelected: $0 == selectedBank)
        }
  ...
  private func getDepositLimit(_ offlinePayment: Observable<PaymentsDTO.Offline>)
    -> SharedSequence<DriverSharingStrategy, AmountRange>
  {
    offlinePayment.map { $0.depositLimit }.asDriverLogError()
  }
  ...
  private func isRemittanceValid(_ depositLimit: Driver<AmountRange>) -> Driver<AmountExpection?> {
    Driver.combineLatest(depositLimit, remittance.asDriverLogError()).map({ limitation, amount -> AmountExpection? in
      ...
      if
        (limitation.min as! AccountCurrency) <= amount.toAccountCurrency(),
        amount.toAccountCurrency() <= (limitation.max as! AccountCurrency)
      {
        return nil
      }
      else {
  ...
  private func isOfflineDataValid(
    userNameValid: Driver<AccountNameException?>,
  ...
  private func confirm() -> Driver<OfflineDepositDTO.Memo> {
    ...
    { selectOfflinePaymentGateway, remitterBank, remitterName, remitterBankCardNumber, amount -> OfflineDepositDTO.Request in
      ...
      let application = OfflineRemitApplication(
        remitter: remitter,
        remittance: amount,
        beneficiaryIdentity: beneficiaryBankCardIdentity)
    ...
    let memo = confirmTrigger.withLatestFrom(request)
      ...
      }.do(onNext: { [weak self] _ in self?.navigator.toDepositOfflineConfirmPage() }).asDriverLogError()
