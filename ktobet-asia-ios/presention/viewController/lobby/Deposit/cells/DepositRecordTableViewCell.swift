...
class DepositRecordTableViewCell: UITableViewCell {
    ...
    func setUp(data: DepositRecord, isOnlyTimeFormat: Bool = false) {
        dateLabel.text = isOnlyTimeFormat ? data.createdDate.formatDateOnlyTime() : data.createdDate.formatDateToStringToSecond()
        ...
        amountLabel.text = data.requestAmount.displayAmount
    }
}
