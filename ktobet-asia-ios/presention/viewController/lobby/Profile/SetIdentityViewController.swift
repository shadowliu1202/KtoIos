...
class SetIdentityViewController: LobbyViewController {
  ...
  private func setCountDownTimer() {
    countDownTimer.start(timeInterval: 1, duration: LockSeconds) { [weak self] _, countDownSecond, _ in
  ...
  private func alertExceedResendLimit() {
    Alert.shared.show(
      ...
      Localize.string("profile_otp_exceed_otp_limit"),
...
class SetIdentityFactory {
  ...
  private func profileExistEmailArgs() -> SetIdentityArgs {
    SetIdentityBuilder(
      failedType: ProfileEmailFailedType(title: Localize.string("profile_identity_email_modify_fail")),
