...
class ProfileViewController: LobbyViewController, AuthProfileVerification {
    ...
    private func dataBinding() {
        self.rx.viewWillAppear.flatMap({ [unowned self](_) in
            return self.viewModel.fetchPlayerProfile()
        }).subscribe(onError: { [weak self] in
            self?.handleErrors($0)
        }).disposed(by: disposeBag)
        
        self.rx.viewWillAppear.flatMap({ [unowned self](_) in
            return self.viewModel.profileAuthorization
        ...
        self.rx.viewWillAppear.flatMap({ [unowned self](_) in
            return self.viewModel.playerProfile
        }).subscribe(onNext: { [weak self] in
            self?.setUpUI($0)
        }).disposed(by: disposeBag)
        
        Observable.combineLatest(self.viewModel.playerProfile, self.viewModel.isAnyWithdrawalTicketApplying.asObservable()).subscribe(onNext: { [weak self] (profile, applyIsOnGoing) in
...
struct EmailAccountType: AccountTypeProtocol {
    ...
    func navigateToConfirmPage(_ identity: String) {
        let oldAccountModifyConfirmationViewController = UIStoryboard(name: "Profile", bundle: nil).instantiateViewController(withIdentifier: "OldAccountModifyConfirmationViewController") as! OldAccountModifyConfirmationViewController
