...
class CasinoDetailRecord2TableViewCell: UITableViewCell {
    ...
    func setup(index: Int, detail: CasinoDetail) {
        ...
        if index == 4 {
            ...
            self.contentLabel.text = detail.stakes.amount.currencyFormatWithoutSymbol(precision: 2, roundingMode: .down)
        }
        
        if index == 5 {
            self.titleLabel.text = Localize.string("product_bet_win_lose")
            if detail.winLoss.amount >= 0 {
                self.contentLabel.text = String(format: Localize.string("product_winning_amount"), abs(detail.winLoss.amount).currencyFormatWithoutSymbol(precision: 2, roundingMode: .down))
                self.contentLabel.textColor = UIColor.textSuccessedGreen
            } else {
                self.contentLabel.text = String(format: Localize.string("product_losing_amount"), abs(detail.winLoss.amount).currencyFormatWithoutSymbol(precision: 2, roundingMode: .down))
            }
        }
    }
}
