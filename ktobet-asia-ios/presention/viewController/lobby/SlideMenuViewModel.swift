...
class SlideMenuViewModel {
    ...
    let arrProducts = Observable<[ProductItem]>.from(optional: {
        var titles = [Localize.string("common_sportsbook"), Localize.string("common_casino"), Localize.string("common_slot"), Localize.string("common_keno"), Localize.string("common_p2p")]
        var imgs = ["SBK", "Casino", "Slot", "Number Game", "P2P"]
        var type : [ProductType] = [.sbk, .casino, .slot, .numbergame, .p2p]
        var arr = [ProductItem]()
