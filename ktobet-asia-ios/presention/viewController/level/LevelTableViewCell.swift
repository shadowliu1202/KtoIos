...
class LevelTableViewCell: UITableViewCell {
    ...
    func loadPrivilegeView(_ privileges: [LevelPrivilege],  _ tapPrivilegeHandler: @escaping (LevelPrivilege) -> ()) {
        privileges.forEach({
            stackView.addArrangedSubview(UnlockPrivilegeView($0, tapPrivilege: tapPrivilegeHandler))
        })
    }
