...
class LevelPrivilegeViewController: APPViewController {
    ...
    func updateCollapse(tableView: UITableView, item: Item, collapse: Observable<Void>, disposeBag: DisposeBag) -> Void {
    ...
    private func getItem(_ row: Int) -> Item {
        return self.resource.value[row]
    }
    
    private func isTopLevel(_ row: Int) -> Bool {
        return row == 0 && getItem(row).level == TopLevel
    }
    
    private func isPreviewLevel(_ row: Int) -> Bool {
        return row == 0 && getItem(row).level != currentLevel
    }
    
    private func isZeroLevel(_ row: Int) -> Bool {
        return row == self.resource.value.count - 1
    }
