...
class CustomerServiceHistoryViewModel {
    ...
    private var historiesTotalCount: Int = 0 {
        didSet {
    ...
    private var chatHistoryTotalSize: BehaviorRelay<Int> = BehaviorRelay<Int>(value: 0)
    ...
    func hasNext(_ lastIndex: Int) -> Bool {
        return lastIndex < historiesTotalCount - 1
    }
    ...
    private func getchatHistory(_ page: Int) -> Single<(TotalCount, [ChatHistory])> {
        return historyUseCase.getChatHistorySummaries(page: page)
    }
    ...
    lazy var deleteCount = Observable.combineLatest(deleteMode, chatHistoryTotalSize, selectedHistory).map({ (mode, totalSize, selection) -> Int in
        switch mode {
        case .include:
    ...
    lazy var isDeleteValid = Observable.combineLatest(deleteMode, chatHistoryTotalSize, selectedHistory).map({ (mode, totalSize, selection) -> Bool in
        switch mode {
        case .include:
    ...
    lazy var isAllHistorySelected = Observable.combineLatest(deleteMode, chatHistoryTotalSize, selectedHistory).map({ (mode, totalSize, selection) -> Bool in
        switch mode {
        case .include:
            return totalSize == selection.count
        case .exclude:
    ...
    func deleteChatHistory() -> Completable {
        return historyUseCase.deleteChatHistories(chatHistories: selectedHistory.value, isExclude: deleteMode.value == .exclude)
    }
