  ...
  override func viewDidLoad() {
    ...
    viewModel.getChatHistory(roomId: roomId)
      ...
      .bind(to: tableView.rx.items) { [weak self] tableView, row, element in
        guard let self, let message = element as? ChatMessage.Message else { return UITableViewCell() }
        ...
        case is PortalChatRoom.SpeakerSystem:
          if self.dataCount - 1 == row, self.dataCount > 1 {
            ...
            cell = tableView
              .dequeueReusableCell(withIdentifier: "\(SystemDialogTableViewCell.self)") as! SystemDialogTableViewCell
