...
class SignupEmailViewController: LandingViewController {
  ...
  func resendTimer(launch: Bool) {
    if launch {
      timerResend
        .start(timeInterval: 1, duration: Setting.resendOtpCountDownSecond, block: { [weak self] _, second, _ in
  ...
  func verifyTimer(launch: Bool) {
    if launch {
      timerVerify
        .repeat(timeInterval: 5, block: { [weak self] _ in
    ...
    NotificationCenter
      ...
      .addObserver(
        ...
        using: { [weak self] _ in
          #if QAT
            if
              let btn = self?.btnQatCancelAutoVerify,
  ...
  @IBAction
  func btnEmailPressed(_: UIButton) {
    if
  ...
  @objc
  private func btnQatCancelAutoVerifyPressed(_ sender: UIButton) {
