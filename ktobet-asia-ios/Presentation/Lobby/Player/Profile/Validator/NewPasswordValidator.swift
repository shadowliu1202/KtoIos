...
class NewPasswordValidator {
  ...
  lazy var passwordValidationError: Observable<UserInfoStatus> = self.accountPassword
    .flatMapLatest({ [unowned self] passwordText in
      self.confirmPassword.asObservable().map({ [unowned self] confirmPasswordText -> UserInfoStatus in
        self.verifyPassword(passwordText: passwordText, confirmPasswordText: confirmPasswordText)
      })
    })
