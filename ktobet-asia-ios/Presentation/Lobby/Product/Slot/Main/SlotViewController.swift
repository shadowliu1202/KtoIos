...
class SlotViewController: ProductsViewController {
  ...
  lazy var gameDataSourceDelegate = ProductGameDataSourceDelegate(self, cellType: .loadMore)
  lazy var newGameDataSourceDelegate = ProductGameDataSourceDelegate(self, cellType: .loadMore)
  lazy var jackpotGameDataSourceDelegate = ProductGameDataSourceDelegate(self, cellType: .loadMore)
  ...
  private func bindingData() {
    ...
    loadMoreCollectionView(
      delegate: gameDataSourceDelegate,
      collectionView: recentlyCollectionView,
      getGame: viewModel.recentGames,
      containerView: recentlyView,
      containerViewHeight: recentlyViewHeight)
    loadMoreCollectionView(
      delegate: newGameDataSourceDelegate,
      collectionView: newCollectionView,
      getGame: viewModel.newGames,
      containerView: newView,
      containerViewHeight: newViewHeight)
    loadMoreCollectionView(
      delegate: jackpotGameDataSourceDelegate,
      collectionView: jackpotCollectionView,
  ...
  private func loadMoreCollectionView(
    delegate: ProductGameDataSourceDelegate,
    collectionView: UICollectionView,
    getGame: Observable<[SlotGame]>,
    containerView: UIView,
    containerViewHeight: NSLayoutConstraint)
  {
      ...
      .subscribe { [weak self] slotGames in
        ...
        if slotGames.count == 0 {
          ...
          delegate.lookMoreTap = { [weak self] in
            self?.performSegue(
              withIdentifier: SlotSeeMoreViewController.segueIdentifier,
              sender: (collectionView.tag, slotGames))
          }
...
extension SlotViewController: BarButtonItemable {
  func pressedRightBarButtonItems(_ sender: UIBarButtonItem) {
    ...
    case is RecordBarButtonItem:
      guard
        let slotSummaryViewController = UIStoryboard(name: "Slot", bundle: nil)
          .instantiateViewController(withIdentifier: "SlotSummaryViewController") as? SlotSummaryViewController
      else { return }
