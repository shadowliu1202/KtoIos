...
class SideBarViewController: APPViewController {
    ...
    private func initUI() {
        ...
        let logoImageView = UIImageView(image: Configuration.current.navigationIcon())
    ...
    private func playerInfoBinding(_ sideMenuViewModel: SideMenuViewModel) {
        ...
        sideMenuViewModel.playerInfo.asObservable()
            ...
            .subscribe(onSuccess: { [unowned self] in
                ...
                // problem where the viewController isn't released. Even manually setting `sideMenuViewModel` to nil doesn't
                // release it
                // because the strong reference in the closure keeps it alive. The current implementation avoids this strong
                // reference
                // issue.
