...
extension SurveyViewController: UITableViewDataSource, UITableViewDelegate {
  ...
  func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    ...
    return tableView.dequeueReusableCell(withIdentifier: "ServeyHeaderViewCell", cellType: ServeyHeaderViewCell.self)
      .configure(questionSection).contentView
  }
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    switch surveyQuestionType {
    case .simpleoption:
      cell = tableView.dequeueReusableCell(withIdentifier: "DropdownOptionCell", cellType: DropdownOptionCell.self)
        .configure(dataSource[section].surveyQuestionOptions, nil) { [weak self] selected in
    ...
    case .multipleoption:
      ...
      cell = tableView.dequeueReusableCell(withIdentifier: "MultipleOptionCell", cellType: MultipleOptionCell.self)
        .configure(option, rowIsSelected(option: option, indexPath: indexPath))
      drawDivider(cell)
    case .textfield:
      cell = tableView.dequeueReusableCell(withIdentifier: "TextFieldCell", cellType: TextFieldCell.self)
        .configure(viewModel: viewModel) { [weak self] text in
  ...
  private func answer(textField text: String, indexPath: IndexPath) {
    ...
    if let question = answerItem?.question {
      if text.isEmpty {
        ...
        let option = SurveyQuestion_.SurveyQuestionOption(
          optionId: "",
...
class DropdownOptionCell: UITableViewCell {
  ...
  func configure(
    _ item: [SurveyQuestion_.SurveyQuestionOption],
    _ theSelected: SurveyQuestion_.SurveyQuestionOption?,
    _ callback: ((SurveyQuestion_.SurveyQuestionOption) -> Void)? = nil)
    -> Self
  {
    self.selectionStyle = .none
    optionDropDown.customizeBackgroundColor = (.greyScaleList, .greyScaleList)
    ...
    optionDropDown.didSelect(completion: { _, index, _ in
...
class TextFieldCell: UITableViewCell, UITextViewDelegate {
  ...
  func configure(viewModel: SurveyViewModel, _ callback: ((String) -> Void)? = nil) -> Self {
    self.selectionStyle = .none
    self.textView.delegate = self
    self.textView.text = Localize.string("customerservice_offline_survey_hint")
    self.textView.textColor = UIColor.textPrimary
