...
class CustomerServiceHistoryEditViewController: LobbyViewController {
  ...
  private func addHeaderView() {
    ...
    button.setTitleColor(UIColor.complementaryDefault, for: .normal)
  ...
  private func dataBinding() {
    ...
    Observable.combineLatest(viewModel.isAllHistorySelected, viewModel.deleteCount)
      .subscribe(onNext: { [weak self] isAllselected, count in
        var title = ""
        if isAllselected {
...
extension CustomerServiceHistoryEditViewController: UITableViewDelegate, UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    let cell = tableView.dequeueReusableCell(withIdentifier: "EditChatHistoryCell", cellType: EditChatHistoryCell.self)
      .configure(item: self.records[indexPath.row], deleteMode: self.deleteMode, selectedHistory: self.selecteds)
    cell.removeBorder()
    if indexPath.row != 0 {
      cell.addBorder()
    }
