...
class TickCircle {
  ...
  private func setTimer(countDownSecond: Int) {
    countDownTimer?
      .start(timeInterval: 1, duration: TimeInterval(countDownSecond)) { [weak self] _, countDownSecond, finish in
  ...
  private func drawTicks(count: Int) {
    ...
    for i in 0..<count {
      ...
      let x =
        CGFloat(
          Float(circle.center.x) + Float(radius) *
            cosf(2 * Float(i) * Float(Double.pi) / Float(count) - Float(Double.pi) / 2))
      let y =
        CGFloat(
          Float(circle.center.y) + Float(radius) *
            sinf(2 * Float(i) * Float(Double.pi) / Float(count) - Float(Double.pi) / 2))
