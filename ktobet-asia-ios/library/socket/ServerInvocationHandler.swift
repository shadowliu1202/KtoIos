...
internal class InvocationHandler<T: Decodable>: ServerInvocationHandler {
  ...
  init(logger: SocketLogger, invocationDidComplete: @escaping (T?, Error?) -> Void) {
    self.logger = logger
  ...
  func processStreamItem(streamItemMessage _: StreamItemMessage) -> Error? {
    logger.log(logLevel: .error, message: "Stream item message received for non-streaming server side method")
    return SignalRError
      .protocolViolation(
        underlyingError: SignalRError
          .invalidOperation(message: "Stream item message received for non-streaming server side method"))
  }
...
internal class StreamInvocationHandler<T: Decodable>: ServerInvocationHandler {
  ...
  init(logger: SocketLogger, streamItemReceived: @escaping (T) -> Void, invocationDidComplete: @escaping (Error?) -> Void) {
    self.logger = logger
