...
public class DefaultReconnectPolicy: ReconnectPolicy {
  ...
  // - parameter retryIntervals: an array of retry intervals. If not provided the following intervals will be used 0, 2, 10 and 30
  // seconds.
  // - note: when providing own `retryIntervals` the client will attempt to re-establish the connection at most as many times as
  // the number of provided intervals. If this fails the connection will be stopped.
