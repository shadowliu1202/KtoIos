...
public class HubConnectionBuilder {
  ...
  private var transportFactory: ((SocketLogger, TransportType) -> TransportFactory) = { logger, permittedTransportTypes in
  ...
  public func withHttpConnectionOptions(configureHttpOptions: (_ httpConnectionOptions: HttpConnectionOptions) -> Void)
  ...
  /// - note: By default logging is disabled. When using this overload all log entries whose level is greater or equal than
  /// `minLogLevel` will be written using the `print` function.
  ...
  ///
  /// The log entries sent to the custom logger will be prefiltered and the logger will receive only the entries whose whose log
  /// level is greator or equal than `minLogLevel`.
  ///
  /// - parameter minLogLevel: minimum log level
  /// - parameter logger: custom logger
  ...
  /// Sets which transport types are turned on. Defaults to all types availble. Currently, only websockets and long polling are
  /// implemented.
  ...
  internal func withCustomTransportFactory(transportFactory: @escaping (SocketLogger, TransportType) -> TransportFactory)
    -> HubConnectionBuilder
  {
  ...
  private func createLegacyHttpConnection(transportFactory: TransportFactory) -> HttpConnection {
    if !(reconnectPolicy is NoReconnectPolicy) {
      logger.log(
        logLevel: .error,
        message: "Using reconnect with legacy HttpConnection is not supported. Ignoring reconnect settings.")
    }
...
extension HubConnectionBuilder {
  ...
  public func withJSONHubProtocol() -> HubConnectionBuilder {
    self.withHubProtocol(hubProtocolFactory: { logger in JSONHubProtocol(logger: logger) })
  }
}
