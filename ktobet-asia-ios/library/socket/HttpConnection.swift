...
public class HttpConnection: Connection {
  ...
  public convenience init(
    ...
    logger: SocketLogger = NullLogger())
  {
  ...
  public func start() {
    ...
    if options.skipNegotiation {
      transport = try! self.transportFactory
        .createTransport(availableTransports: [TransportDescription(
      ...
      negotiate(negotiateUrl: createNegotiateUrl(), accessToken: nil) { negotiationResponse in
        do {
          self.transport = try self.transportFactory
            .createTransport(availableTransports: negotiationResponse.availableTransports)
        }
        catch {
  ...
  private func negotiate(
    negotiateUrl: URL,
    accessToken: String?,
    negotiateDidComplete: @escaping (NegotiationResponse) -> Void)
  {
    if let accessToken {
    ...
    httpClient.post(url: negotiateUrl, body: nil) { httpResponse, error in
      ...
      if httpResponse.statusCode == 200 {
        ...
        do {
          ...
          self.logger.log(
            logLevel: .debug,
            message: "Negotiate response: \(payload != nil ? String(data: payload!, encoding: .utf8) ?? "(nil)" : "(nil)")")
          ...
          case let redirection as Redirection:
            ...
            self.negotiate(
              ...
              accessToken: redirection.accessToken,
