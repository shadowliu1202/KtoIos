...
/// `HubConnection` is the client for interacting with SignalR server. It allows invoking server side hub methods and register
...
public class HubConnection {
  ...
  /// time the connection is successfully started.
  ...
  public init(connection: Connection, hubProtocol: HubProtocol, logger: SocketLogger = NullLogger()) {
  ...
  /// ```
  /// hubConnection.stream(method: "StreamNumbers", arguments: [10, 1], streamItemReceived: { (item: Int) in print("\(item)" }) {
  ...
  private func invoke(invocationHandler: ServerInvocationHandler, method: String, arguments: [Encodable]) -> String {
...
private class HubConnectionConnectionDelegate: ConnectionDelegate {
  ...
  func connectionDidOpen(connection _: Connection) {
  ...
  func connectionDidReceiveData(connection _: Connection, data: Data) {
