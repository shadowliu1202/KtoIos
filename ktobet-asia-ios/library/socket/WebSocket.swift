...
private class Payload {
  ...
  var nsdata: Data {
    get {
      Data(bytes: ptr.assumingMemoryBound(to: UInt8.self), count: count)
    }
    set {
      count = 0
  ...
  var buffer: UnsafeBufferPointer<UInt8> {
    get {
      UnsafeBufferPointer<UInt8>(start: ptr.assumingMemoryBound(to: UInt8.self), count: count)
    }
    set {
      count = 0
...
public struct WebSocketEvents {
  /// An event to be called when the WebSocket connection's readyState changes to .Open; this indicates that the connection is
  /// ready to send and receive data.
  public var open: () -> Void = { }
  /// An event to be called when the WebSocket connection's readyState changes to .Closed.
  public var close: (_ code: Int, _ reason: String, _ wasClean: Bool) -> Void = { _, _, _ in }
  /// An event to be called when an error occurs.
  public var error: (_ error: Error) -> Void = { _ in }
  /// An event to be called when a message is received from the server.
  public var message: (_ data: Any) -> Void = { _ in }
  /// An event to be called when a pong is received from the server.
  public var pong: (_ data: Any) -> Void = { _ in }
  /// An event to be called when the WebSocket process has ended; this event is guarenteed to be called once and can be used as an
  /// alternative to the "close" or "error" events.
  public var end: (_ code: Int, _ reason: String, _ wasClean: Bool, _ error: Error?) -> Void = { _, _, _, _ in }
}

/// The WebSocketBinaryType enum is used by the binaryType property and indicates the type of binary data being transmitted by the
...
public enum WebSocketBinaryType: CustomStringConvertible {
  ...
  case nsData
  /// The WebSocket should transmit UnsafeBufferPointer<UInt8> objects. This buffer is only valid during the scope of the message
  /// event. Use at your own risk.
...
/// The WebSocketCompression struct is used by the compression property and manages the compression options for the WebSocket
/// connection.
...
public struct WebSocketService: OptionSet {
  ...
  public init(nilLiteral _: ()) { self.value = 0 }
  public static var allZeros: WebSocketService { self.init(0) }
  static func fromMask(_ raw: UInt) -> WebSocketService { self.init(raw) }
  public var rawValue: UInt { self.value }
  /// No services.
  public static var None: WebSocketService { self.init(0) }
  /// Allow socket to handle VoIP.
  public static var VoIP: WebSocketService { self.init(1 << 0) }
  /// Allow socket to handle video.
  public static var Video: WebSocketService { self.init(1 << 1) }
  /// Allow socket to run in background.
  public static var Background: WebSocketService { self.init(1 << 2) }
  /// Allow socket to handle voice.
  public static var Voice: WebSocketService { self.init(1 << 3) }
}
...
public enum WebSocketError: Error, CustomStringConvertible {
  ...
  public var description: String {
    ...
    case .invalidResponse(let details): return "InvalidResponse(\(details))"
    case .invalidCompressionOptions(let details): return "InvalidCompressionOptions(\(details))"
    case .libraryError(let details): return "LibraryError(\(details))"
    case .protocolError(let details): return "ProtocolError(\(details))"
    case .payloadError(let details): return "PayloadError(\(details))"
    case .network(let details): return "Network(\(details))"
    }
  }
...
private class UTF8 {
  ...
  func append(_ byte: UInt8) throws {
    ...
    if procd == count {
      ...
      procd = 0
      count = 0
      text.append(String(describing: UnicodeScalar(codepoint)!))
    }
  }
  ...
  static func bytes(_ string: String) -> [UInt8] {
    ...
    return [UInt8](UnsafeBufferPointer<UInt8>(
...
private func zlibVersion() -> OpaquePointer
@_silgen_name("deflateInit2_")
private func deflateInit2(
  ...
  memLevel: CInt,
  strategy: CInt,
  version: OpaquePointer,
  stream_size: CInt) -> CInt
@_silgen_name("deflateInit_")
private func deflateInit(
  _ strm: UnsafeMutableRawPointer,
  level: CInt,
  version: OpaquePointer,
  stream_size: CInt) -> CInt
@_silgen_name("deflateEnd")
private func deflateEnd(_ strm: UnsafeMutableRawPointer) -> CInt
@_silgen_name("deflate")
private func deflate(_ strm: UnsafeMutableRawPointer, flush: CInt) -> CInt
...
private func inflateInit(
  _ strm: UnsafeMutableRawPointer,
  version: OpaquePointer,
  stream_size: CInt) -> CInt
@_silgen_name("inflate")
private func inflateG(_ strm: UnsafeMutableRawPointer, flush: CInt) -> CInt
@_silgen_name("inflateEnd")
private func inflateEndG(_ strm: UnsafeMutableRawPointer) -> CInt
...
private class Inflater {
  ...
  init?(windowBits: Int) {
    ...
    let ret = inflateInit2(
      &strm,
      windowBits: -CInt(windowBits),
      version: zlibVersion(),
      stream_size: CInt(MemoryLayout<z_stream>.size))
    if ret != 0 {
      return nil
    }
  }
...
private class Deflater {
  ...
  /// func deflate(_ bufin : UnsafePointer<UInt8>, length : Int, final : Bool) -> (p : UnsafeMutablePointer<UInt8>, n : Int, err :
  /// NSError?){
  // return (nil, 0, nil)
  // }
}

/// WebSocketDelegate is an Objective-C alternative to WebSocketEvents and is used to delegate the events for the WebSocket
/// connection.
@objc
public protocol WebSocketDelegate {
  /// A function to be called when the WebSocket connection's readyState changes to .Open; this indicates that the connection is
  /// ready to send and receive data.
  func webSocketOpen()
  ...
  /// A function to be called when the WebSocket process has ended; this event is guarenteed to be called once and can be used as
  /// an alternative to the "close" or "error" events.
  @objc
...
private class InnerWebSocket: Hashable {
  ...
  var outputBytesSize = 0
  var outputBytesStart = 0
  var outputBytesLength = 0
  var inputBytes: UnsafeMutablePointer<UInt8>?
  var inputBytesSize = 0
  var inputBytesStart = 0
  var inputBytesLength = 0
  ...
  var subProtocol: String { privateSubProtocol }
  ...
  var readyState: WebSocketReadyState { privateReadyState }
        ...
        case .binary:
          fire {
            ...
            case .nsData:
              ...
              // The WebSocketDelegate is necessary to add Objective-C compability and it is only possible to send binary data
              // with NSData.
      ...
      case .closeConn:
        ...
        if rd != nil {
          ...
          fire {
            ...
            self.eventDelegate?.webSocketClose(
              Int(self.closeCode),
              reason: self.closeReason,
              wasClean: self.closeFinal)
          }
        }
        stage = .end
      case .end:
        fire {
          ...
          self.eventDelegate?.webSocketEnd?(
            Int(self.closeCode),
            reason: self.closeReason,
            wasClean: self.closeClean,
            error: self.finalError as NSError?)
        }
        exit = true
        manager.remove(self)
      }
    ...
    if rd != nil {
      ...
      if more {
        while rd.hasBytesAvailable {
          ...
          let n = rd.read(
            inputBytes! + inputBytesStart + inputBytesLength,
            maxLength: inputBytesSize - inputBytesStart - inputBytesLength)
          if n > 0 {
            inputBytesLength += n
          }
        }
  ...
  func openConn() throws {
    ...
    let rkey = Data(bytes: UnsafePointer(keyb), count: 16)
  ...
  func readResponse() throws {
    ...
    var key = ""
    let trim: (String) -> (String) = { text in text.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines) }
    let eqval: (String, String) -> (String) = { line, del in trim(line.components(separatedBy: del)[1]) }
    ...
    if fragStateSaved {
      ...
      if leaderCode == .close {
        ...
        if len >= 2 {
          ...
          len -= 2
          if
            statusCode < 1000 || statusCode > 4999 || (statusCode >= 1004 && statusCode <= 1006) ||
...
open class WebSocket: NSObject {
  ...
  /// Create a WebSocket connection to a URL; this should be the URL to which the WebSocket server will respond. Also include a
  /// list of protocols.
  ...
  open var subProtocol: String { ws.subProtocol }
  ...
  /// Closes the WebSocket connection or connection attempt, if any. If the connection is already closed or in the state of
  /// closing, this method does nothing.
  ///
  /// :param: code An integer indicating the status code explaining why the connection is being closed. If this parameter is not
  /// specified, a default value of 1000 (indicating a normal closure) is assumed.
  /// :param: reason A human-readable string explaining why the connection is closing. This string must be no longer than 123
  /// bytes of UTF-8 text (not characters).
