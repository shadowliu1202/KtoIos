...
open class OpalImagePickerRootViewController: UIViewController {
    ...
    open var maximumImageSizeLimit: Int = -1
    ...
    open override func viewDidLoad() {
        ...
        navigationItem.title = configuration?.navigationTitle ?? NSLocalizedString("", comment: "")
        
        let cancelButtonTitle = configuration?.cancelButtonTitle ?? Localize.string("common_cancel")
        ...
        let doneButtonTitle = configuration?.doneButtonTitle ?? Localize.string("common_done")
    ...
    @objc func doneTapped() {
        ...
        for indexPath in indexPathsForSelectedItems {
            guard indexPath.item < self.photoAssets.count + 1 else { continue }
            photoAssets += [self.photoAssets.object(at: indexPath.item - 1)]
        }
...
//            let message = configuration?.maximumSelectionsAllowedMessage ?? NSLocalizedString("You cannot select more than \(maximumSelectionsAllowed) images. Please deselect another image before trying to select again.", comment: "You cannot select more than (x) images. Please deselect another image before trying to select again. (OpalImagePicker)")
            let message = Localize.string("common_photo_upload_count_limit")
        ...
        guard let imageData = cell.imageView.image?.jpegData(compressionQuality: 1.0) else { return false }
        if imageData.count > maximumImageSizeLimit {
            let message = Localize.string("deposit_execeed_limitation")
            let alert = UIAlertController(title: "", message: message, preferredStyle: .alert)
            let okayString = configuration?.okayString ?? NSLocalizedString("OK", comment: "OK")
            let action = UIAlertAction(title: okayString, style: .cancel, handler: nil)
            alert.addAction(action)
...
extension OpalImagePickerRootViewController: UICollectionViewDataSource {
    ...
    private func photoAssetCollectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row == 0 {
            cell.imageView.image = UIImage(named: "iconActionTakePhoto32")
            return cell
        }
        let photoAsset = photoAssets.object(at: indexPath.item - 1)
