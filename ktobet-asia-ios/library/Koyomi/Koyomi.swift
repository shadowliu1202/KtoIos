...
final public class Koyomi: UICollectionView {
    ...
    public var style: KoyomiStyle = .standard {
        didSet {
            dayBackgrondColor  = .black_two //style.colors.dayBackgrond
            weekBackgrondColor = .black_two//style.colors.weekBackgrond
            weekColor = .black_two//style.colors.week
            weekdayColor = .black_two//style.colors.weekday
    ...
    @IBInspectable public var currentDateFormat: String = "yyyy/M"
    ...
    func configure(_ cell: KoyomiCell, at indexPath: IndexPath) {
        ...
        cell.textColor = {
            if isSelected {
                ...
                if cell.xLabel.text != "" {
                    return UIColor.textSecondaryScorpionGray
                } else {
                    if indexPath.section == 0 {
                        return UIColor.yellowFull
                    } else {
                        if let begin = model.indexAtBeginning(in: .current),
        ...
        cell.configureAppearanse(of: style, withColor: selectionColor, backgroundColor: backgroundColor, isSelected: isSelected, model: model)
    }
}
