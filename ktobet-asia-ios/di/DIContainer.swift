...
class DIContainer {
    ...
    func registViewModel(){
        ...
        ctner.register(SurveyViewModel.self) { (resolver) in
            return SurveyViewModel(ctner.resolve(CustomerServiceSurveyUseCase.self)!, ctner.resolve(AuthenticationUseCase.self)!, ctner.resolve(LocalStorageRepository.self)!)
        }
