...
extension SurveyAnswers {
    func toSurveyAnswerBean() -> [SurveyAnswerBean] {
        return self.answers.map({ (question, options) in
            SurveyAnswerBean(questionID: question.questionId, questionText: question.description_, answer: options.map({$0.toAnswerBean()}))
        })
    }
}
