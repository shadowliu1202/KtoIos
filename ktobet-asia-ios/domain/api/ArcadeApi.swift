...
class ArcadeApi: ApiService, WebGameApi {
    ...
    func getGameRecordByDate(date: String, offset: Int32, skip: Int, take: Int) -> Single<ResponseData<ArcadeDateBetRecordBean>> {
        let target = GetAPITarget(service: self.url("\(prefix)/wager/mobile/mybet/gamegroup")).parameters(["date": date, "offset": offset, "skip": skip, "take": take])
        return httpClient.request(target).map(ResponseData<ArcadeDateBetRecordBean>.self)
    }
    
    func getBetRecords(beginDate: String, endDate: String, gameId: Int32, skip: Int, take: Int) -> Single<ResponseData<ArcadeGameBetRecordDataBean>> {
        let target = GetAPITarget(service: self.url("\(prefix)/wager/mobile/mybet/list-by-paging")).parameters(["beginDate": beginDate, "endDate": endDate, "gameId": gameId, "skip": skip, "take": take])
