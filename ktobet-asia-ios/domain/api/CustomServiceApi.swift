...
class CustomServiceApi: ApiService, ImageApiProtocol {
  ...
  func getInProcessInformation(roomId _: String) -> Single<ResponseData<[InProcessBean]>> {
    let target = APITarget(
      baseUrl: httpClient.host,
  ...
  func closeChatRoom(roomId _: String) -> Completable {
    let target = APITarget(
      baseUrl: httpClient.host,
  ...
  func connectSurveyWithRoom(connectId: String, roomId: String) -> Completable {
    let target = PostAPITarget(
      service: self.url("\(prefix)/survey/set-survey-room/\(connectId)/\(roomId)"),
      parameters: Empty())
    return httpClient.request(target).asCompletable()
  }
