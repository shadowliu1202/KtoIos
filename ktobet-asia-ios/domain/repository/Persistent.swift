...
let suiteName: String = "Persistent"

@propertyWrapper
struct Persistent<T: PropertyListValue> {
    let key: Key
    
    private lazy var defaults = UserDefaults.init(suiteName: suiteName)
    ...
    var projectedValue: Persistent<T> { return self }
    
    func observe(change: @escaping (T?, T?) -> Void) -> NSObject {
        return UserDefaultsObservation(suiteName: suiteName, key: key) { old, new in
...
struct PersistentObject<T: Codable> {
    ...
    var projectedValue: PersistentObject<T> { return self }
