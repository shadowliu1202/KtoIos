...
class DepositAdapter: DepositProtocol {
  ...
  func putDepositImages(displayId: String, imageMappingRequest: ImageMappingRequestBean) -> SharedBu.Completable {
    ...
    let dataBean = UploadImagesData(ticketStatus: imageMappingRequest.ticketStatus, images: imgBeans)
  ...
  func getDepositDetail(displayId: String) -> SingleWrapper<ResponseItem<DepositDetailBean>> {
    self.bankAPI.getDepositRecordDetail(displayId: displayId)
  ...
  {
    self.bankAPI.getDepositRecords(page: page, deteBegin: begin, dateEnd: end, status: statusMap)
  ...
  func getDepositMethods(depositTypeId: Int32) -> SingleWrapper<ResponseList<DepositMethodBean>> {
    self.bankAPI.getDepositMethods(depositType: depositTypeId)
  ...
  func getDepositOfflineBankAccounts() -> SingleWrapper<ResponseItem<DepositOfflineBankAccountsBean>> {
    self.bankAPI.getDepositOfflineBankAccounts()
  ...
  func getUpdateOnlineDepositCrypto(displayId: String) -> SingleWrapper<ResponseItem<DepositUrlBean>> {
    self.bankAPI.requestCryptoDetailUpdate(displayId: displayId)
  ...
  func onlineDepositCrypto(request: CryptoDepositRequestBean) -> SingleWrapper<ResponseItem<CryptoDepositResponseBean>> {
    cpsAPI.onlineDepositCrypto(cryptoDepositRequest: CryptoDepositRequest(cryptoCurrency: request.cryptoCurrency))
  ...
  func getCryptoExchangeFeeSetting(cryptoExchange: Int32 = 1) -> SingleWrapper<ResponseItem<FeeSettingBean>> {
    cpsAPI.getCryptoExchangeFeeSettingString(cryptoExchange: cryptoExchange)
