...
class WithdrawalRepositoryImpl: WithdrawalRepository {
    ...
    fileprivate func createWithdrawalRecordDetail(detail: WithdrawalRecordDetailData, transactionTransactionType: Int32) -> Single<WithdrawalRecordDetail> {
        ...
        return getStatusChangeHistories(statusChangeHistories: detail.statusChangeHistories).map { (tHistories) -> WithdrawalRecordDetail in
            ...
            return WithdrawalRecordDetail(record: withdrawalRecord, isBatched: detail.isBatched, isPendingHold: detail.isPendingHold, statusChangeHistories: tHistories, updatedDate: updateOffsetDateTime)
        }
    }
        ...
        return bankApi.getWithdrawalAccount().map({ (response: ResponseData<PayloadPage<WithdrawalAccountBean>>) -> [WithdrawalAccount] in
            if let databeans = response.data?.payload {
                let data: [WithdrawalAccount] = databeans.map {
                    WithdrawalAccount(accountName: $0.accountName, accountNumber: $0.accountNumber, address: $0.address, bankId: Int32($0.bankID), bankName: $0.bankName, branch: $0.branch, city: $0.city, location: $0.location, playerBankCardId: $0.playerBankCardID, status: Int32($0.status), verifyStatus: PlayerBankCardVerifyStatus.Companion.init().create(status: Int32($0.verifyStatus)))
                }
                return data
            }
            return []
