...
infix operator <->: DefaultPrecedence
    ...
    func nonMarkedText(_ textInput: UITextInput) -> String? {
        ...
        guard
            let rangeAll = textInput.textRange(from: start, to: end),
    ...
    func <-> <Base>(textInput: TextInput<Base>, relay: BehaviorRelay<String>) -> Disposable {
        ...
        let bindToRelay = textInput.text
            .subscribe(onNext: { [weak base = textInput.base] _ in
                ...
                /// In some cases `textInput.textRangeFromPosition(start, toPosition: end)` will return nil even though the
                /// underlying
                /// value is not nil. This appears to be an Apple bug. If it's not, and we are doing something wrong, please let
                /// us know.
...
func <-> <T>(property: ControlProperty<T>, relay: BehaviorRelay<T>) -> Disposable {
    if T.self == String.self {
        //
        // #if DEBUG && !os(macOS)
        //        fatalError("It is ok to delete this message, but this is here to warn that you are maybe trying to bind to some
        // `rx.text` property directly to relay.\n" +
        //            "That will usually work ok, but for some languages that use IME, that simplistic method could cause
        //            unexpected
        // issues because it will return intermediate results while text is being inputed.\n" +
